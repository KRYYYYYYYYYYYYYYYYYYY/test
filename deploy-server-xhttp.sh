...
echo ""
echo -e "${CYAN}  Use these values for deploy.sh (DUAL mode):${NC}"
echo ""
echo -e "    Server IP:    $(hostname -I | awk '{print $1}')"
echo -e "    Worker URL:   <your-worker>.workers.dev"
echo -e "    UUID:         ${XHTTP_UUID}"
echo -e "    Secret path:  ${XHTTP_PATH}"
echo ""
echo -e "${CYAN}  Client one-liner:${NC}"
echo -e "  sudo bash -c \"\$(curl -sSfL https://raw.githubusercontent.com/KRYYYYYYYYYYYYYYYYYYY/test/main/deploy.sh)\""
echo -e "  -> Select mode 2 (DUAL)"
echo ""
echo -e "${YELLOW}  Note: Security = none (TLS terminates at Cloudflare).${NC}"
echo -e "${YELLOW}  Worker proxies HTTPS -> HTTP on port ${XHTTP_PORT}.${NC}"
echo -e "${YELLOW}  If you need TLS on this port, configure a certificate in 3X-UI.${NC}"
echo ""
